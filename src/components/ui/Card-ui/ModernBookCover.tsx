...
const colorMap = {
  ...
  orange: { from: 'from-orange-800', to: 'to-orange-500' },
...
export const ModernBookCover = ({
  ...
  return (
    <div
      className={`z-10 group [perspective:800px] w-min [--shadowColor:#111] ${className} h-[400px]`}
    >
      <div
        style={{
      ...
      >
        {/* Front Side */}
        <div
          className={`absolute inset-y-0 h-[400px] overflow-hidden size-full left-0 text-white flex flex-col justify-end p-6 bg-gradient-to-tr  ${gradient.from} ${gradient.to} ${radiusMap[radius]}`}
          style={{
        ...
        {/* Spine */}
        <div
          className='absolute left-0 bg-white h-[400px]'
          style={{
            top: '3px',
            bottom: '3px',
            width: '48px',
        ...
        {/* Back Side */}
        <div
          className={`absolute inset-y-0 overflow-hidden size-full h-[400px] left-0 text-white flex flex-col justify-end p-6 bg-gradient-to-tr ${gradient.from} ${gradient.to} ${radiusMap[radius]}`}
          style={{
            transform: 'translateZ(-25px)',
